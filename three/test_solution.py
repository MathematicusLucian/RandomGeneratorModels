import pytest
from randomchoices import RandomGenRandomChoices
...
def test_constructor():
    ...
    random_gen = RandomGenRandomChoices(__random_nums, __probs)
    
    # Check if _random_nums and _probabilities are set correctly
    assert random_gen._random_nums == random_nums
    assert random_gen._probabilities == probabilities
...
def test_positive_next_num_once__valid_random_num(probs, random_nums):
    random_gen = RandomGenRandomChoices(random_nums,probs)
